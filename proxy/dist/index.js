...
const acceptLedger = new Map();
// helpers
const ok = (res, data) => res.json({ ok: true, ...data });
const err = (res, code, msg, status = 400) => res.status(status).json({ ok: false, code, message: msg, meta: { status } });
/**
 * GET /apiclient/menu
...
app.post("/apiclient/validateOrder", (req, res) => {
    ...
    const bad = draft.items.filter((i) => !allowed.has(i.sku));
    if (bad.length) {
